...
const items = JSON.parse(
  readFileSync(join(__dirname, '../data/kampo-master-additional.json'), 'utf-8')
);
...
async function createPage(item) {
  const body = {
    ...
    properties: {
      ...
      '効能一言': { rich_text: toRichText(item.tagline || '') },
...
async function main() {
  console.log(`追加投入開始: ${items.length}件`);
  let ok = 0, ng = 0;
